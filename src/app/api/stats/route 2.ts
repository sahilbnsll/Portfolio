...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("All entries dates:", entries.map((e) => e.key));

    // Get today's data - explicitly find today's entry by date
    let todayEntry = entries.find((e) => e.key === todayDateStr);
    
    // If today's entry doesn't exist, use the last entry (most recent)
    if (!todayEntry && entries.length > 0) {
      todayEntry = entries[entries.length - 1];
      console.log("Today entry not found by date, using last entry:", todayEntry?.key);
    }
